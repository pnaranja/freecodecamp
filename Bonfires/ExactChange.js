...
function drawer(price, cash, cid) {
  ...
  var denom_value = {'PENNY':0.01, 'NICKEL':0.05, 'DIME':0.10, 'QUARTER':0.25, 'ONE':1, 'FIVE':5, 'TEN':10, 'TWENTY':20, 'ONE HUNDRED':100};

  //Find out largest denomination for change
  var greatest_denom = function(){
    if (change<0.05) return 'PENNY';
    if (change<0.10) return 'NICKEL';
    if (change<0.25) return 'DIME';
  ...
  var denom_change = function(denom_arr){
        ...
        var acc = function(denom_arr, ret_arr){
            if (change <= 0 || denom_arr[1] === 0 || change < denom_value[denom]){ 
                return ret_arr;
                
            }
            console.log(change, denom_value[denom]);
            change = Math.round((change - denom_value[denom])*100)/100;
...
//Main Code
  ...
  return total_change.filter(function(arr){ return arr[1] !== 0});

}


drawer(3.26, 100.00, [['PENNY', 1.01], ['NICKEL', 2.05], ['DIME', 3.10], ['QUARTER', 4.25], ['ONE', 90.00], ['FIVE', 55.00], ['TEN', 20.00], ['TWENTY', 60.00], ['ONE HUNDRED', 100.01]]);
